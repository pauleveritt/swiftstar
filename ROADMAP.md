...
| P24 | Digested first-class tools | P9's deferred condensation-direction attack on prefill-tail cost (measured: one file re-read 31× = 37% of Σsuffix). Deterministic host-owned **tools** (no model, millisecond Swift), in cycles — P24.1 windowed reads (shipped), P24.2 read-guard retirement (shipped), P24.3 the tool ladder (`test`/`scout`/`lint`/mediated bash/model-asks-human, upcoming), then an instrument-reconciliation cleanup cycle. Motivation, naming (tool vs. subagent), and the read-guard's falsification/re-scoping history: [P24 direction and re-scoping](docs/superpowers/research/2026-08-30-p24-direction-and-rescoping.md). | **(1) P24.1 — window-honoring reads.** Closed 2026-08-30, 799 tests; repeat rate 1.45→1.00; control reproduced the loop. [Design](docs/superpowers/specs/2026-08-30-p24-1-window-honoring-reads-design.md), [measurement](docs/superpowers/research/2026-08-30-p24-1-window-honoring-after-measurement.md).<br>**(2) P24.2 — read-guard re-decision.** Decided 2026-08-30: guard does not return; `.pool` `readCache` retired (instrument change, re-baselined). [Design](docs/superpowers/specs/2026-08-30-p24-2-read-guard-redecision-design.md).<br>**(3) P24.3 — the tool ladder.** `test`/`lint`/digested-`bash` shipped (cycles 1–2); engine schemas + latent `[upto]`-test fix landed (fork div #16/#17, pin `1e862d9`); `scout`/policy/ask-human, recapture, paired-bill ahead.<br>**(4) Cleanup — reconcile agenttest with product.** Last; two behavioral divergences (refusal-streak corrective, `toolCallBudget` timing) await keep/drop/port.<br>Generalization arms re-run once P24.3 ships (see Now).  |
