...
struct RunVerbTests {
    ...
    /// `AgentSessionTests.buildFakeEngine`.
    private func buildFakeEngine(capture: String, settings: AgentSettings) throws {
        let argv = AgentCommand.argv(settings: settings)
        let source = try FakeAgentSource.generate(
            capture: Data(capture.utf8), engineArgv: argv, hostTools: true)
