...
struct MetricsReducerTests {
    @Test func ratchetHoldsLastNonZeroRates() {
        ...
        reducer.reduce(&state, .status(StatusSnapshot(ctxUsed: 100, ctxSize: 32768, prefillTPS: 5.8, genTPS: 0, ts: 0)))
        #expect(state.prefillTPS == 5.8)
        #expect(state.genTPS == 0)
        // Generation phase: prefill drops to 0, gen becomes non-zero.
        reducer.reduce(&state, .status(StatusSnapshot(ctxUsed: 200, ctxSize: 32768, prefillTPS: 0, genTPS: 12.3, ts: 0)))
        #expect(state.prefillTPS == 5.8)  // ratcheted — does not flicker to 0
        #expect(state.genTPS == 12.3)
        // Idle: both zero — both hold.
        reducer.reduce(&state, .status(StatusSnapshot(ctxUsed: 200, ctxSize: 32768, prefillTPS: 0, genTPS: 0, ts: 0)))
        #expect(state.prefillTPS == 5.8)
        #expect(state.genTPS == 12.3)
    }
    ...
    @Test func statusUpdatesContext() {
        ...
        reducer.reduce(&state, .status(StatusSnapshot(ctxUsed: 958, ctxSize: 32768, prefillTPS: 0, genTPS: 0, ts: 0)))
