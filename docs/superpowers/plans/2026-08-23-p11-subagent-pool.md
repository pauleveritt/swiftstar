...
- [ ] **Step 4: Refactor to N workers, each with its own thread, serialized by a pool mutex.** Replace the single `agent_worker` with `agent_worker *workers` (size `num_workers`); each `agent_worker_init` already creates its own `ds4_session` + thread, so the per-worker code is reused unchanged (GLM 5.2 review: single-thread multiplexing would require rewriting ~250 `w->session`/`w->transcript`/`w->status` references; N workers + a pool mutex is the lower-risk shape). Add a `pool_mu` held around `worker_run_turn` (whole turn, not eval-only — the `--host-tools` `tool_result` read happens inside the turn, so whole-turn naturally serializes stdin reads), a `generating_worker` pointer set/cleared around the turn (assert it is `-1` on entry), and a `stdin_mu` that the worker blocks on and the main loop trylocks (kills the main-loop-steals-`tool_result` TOCTOU). Each `workers[i]` gets `worker_id = i`; all sessions are created at `cfg->gen.ctx_size` (per-worker small-ctx sizing is a follow-up flag — D8's mechanism, not v1). With `num_workers == 1` the array degenerates to the single-worker path and the pool mutex is uncontended, so N==1 stays byte-identical.
