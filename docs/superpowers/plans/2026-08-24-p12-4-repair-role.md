...
- Repair packet: `role: .repair`, `sampling: .off` by default, ~~`.bounded` when `AGENTTEST_REPAIR_THINK=1`~~. Redaction: validate the **authored** packet; append machine evidence after. Evidence caps: pytest output tail 8192 bytes, per-file 4096 bytes.
  - **As-built correction (2026-08-25):** the `AGENTTEST_REPAIR_THINK` override below (Task 5, and the code at the `let think:` line) **is not what shipped and does not work**. Nothing reads `packet.sampling` at dispatch time — thinking is set once at engine spawn from `AGENTTEST_THINK`, a whole-process property — so the override set the packet's recorded value while the engine ran `--nothink`, i.e. it made the capture lie. As shipped, `repairPacket`'s `think` mirrors `AGENTTEST_THINK` (matching what the engine actually runs) and the harness warns on stderr if the inert variable is set. The evidence caps also changed: per-file is **16384 bytes with middle-truncation** (head+tail kept), not 4096 head-only. See the design's "As-built deviations" section, which is authoritative over this plan.
